(function(context) {
	...
	if (!MTVNPlayer.require) {
		...
		var yepnope = window.yepnope,
			...
			onMany = function(packageNames, callback) {
				var done = function() {
					var result = [];
			 ...
			 */
			onPackage = function(name, callback) {
				if (has(name)) {
					...
					if (packageManifest[name]) {
						var packages = {};
						packages[name] = packageManifest[name];
			...
			shim = function(exports, global, url, result, key) {
				...
				provide(key, context[exports]);
				if (global) {
					// push into global (used for testing, probably should kill it.)
					context[key] = context[exports];
				} else {
					// don't touch global, reset it to what it was before load.
					context[exports] = existingGlobals[exports];
				}
			},
			loadPackages = function(packages, callback) {
				...
				var targetLoad = {
					...
					complete: function() {
						isLoadingPackages = false;
						if (callback) {
							callback.apply(null, getPackageArray(packages));
						}
						if (queue.length > 0) {
							queue.shift()();
						}
		 ...
		 */
		MTVNPlayer.require = require;
		/**
		 * provide a module with the name, or delete a module.
		 * @param name {String} The name of the module
		 * @param module {Object} The module itself. If blank, the modeule with the name will be deleted.
		 */
		MTVNPlayer.provide = provide;
		/**
		 ...
		 */
		MTVNPlayer.listPackages = listPackages;
		/**
		 ...
		 */
		MTVNPlayer.loadPackages = loadPackages;
		MTVNPlayer.configurePackages = configurePackages;
	}
})(window);